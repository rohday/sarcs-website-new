...
export default function Grainient({
  ...
  // WebGL context initialization, render loop, and visibility management
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        iTime: { value: 0 },
        uTimeSpeed: { value: isReducedMotion ? 0 : timeSpeed },
    ...
    const startAnimation = () => {
      ...
      const animate = (currentTime: number) => {
        if (!ctx.isVisible || !ctx.isTabVisible) {
          ctx.animationFrameId = null;
          return;
        }

        const delta = Math.min((currentTime - ctx.lastTime) / 1000, 0.1);
        ctx.lastTime = currentTime;
        ctx.totalTime += delta;

        program.uniforms.iTime.value = ctx.totalTime;
        renderer.render({ scene: mesh });

        ctx.animationFrameId = requestAnimationFrame(animate);
      };
    ...
    const handleVisibilityChange = () => {
      ctx.isTabVisible = !document.hidden;
      if (ctx.isVisible && ctx.isTabVisible) {
        startAnimation();
      } else {
        stopAnimation();
  ...
  // Sync uniform properties with prop changes and respect prefers-reduced-motion
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    ...
    u.uTimeSpeed.value = isReducedMotion ? 0 : timeSpeed;
    ...
    u.uCenterOffset.value = [centerX, centerY];
    u.uZoom.value = zoom;
